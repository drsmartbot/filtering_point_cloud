# Reference from here: https://towardsdatascience.com/neighborhood-analysis-kd-trees-and-octrees-for-meshes-and-point-clouds-in-python-19fa96527b77
...
if __name__ == '__main__':
    ...
    df = pd.concat([pd.DataFrame(points,columns=["x","y","z"]), pd.DataFrame(normals,columns=["nx","ny","nz"]), pd.DataFrame(colors,columns=["red","green","blue"])],axis=1)
    
    dbscan_ = DBSCAN(eps=0.3, min_samples=100)
    dbscan_.fit(df.iloc[:,:3])
    result = df[df["outlier"]==0].drop(columns=["outlier"],axis=1)
    point_cloud = o3d.geometry.PointCloud()
    point_cloud.points = o3d.utility.Vector3dVector(np.array(result.iloc[:,:3]))
    point_cloud.colors = o3d.utility.Vector3dVector(np.array(result.iloc[:,3:6]))
    point_cloud.normals = o3d.utility.Vector3dVector(np.array(result.iloc[:,6:9]))
